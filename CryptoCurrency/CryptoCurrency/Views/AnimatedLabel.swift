...
//
// Source https://github.com/dataxpress/UICountingLabel/
enum CountingMethod {
...
enum DecimalPoints {
    ...
    var format: String {
        ...
        case .one: return "%.1f"
        case .two: return "%.2f"
        case .infinity: return "%f"
        }
    }
}

enum SuffixLabel: String {
    case percentage = "%"
    case dollar = "$"
}
...
final class AnimatedLabel: UILabel {
    ...
    var suffix: SuffixLabel = .percentage
    ...
    fileprivate func setTextValue(value: Float) {
        text = String(format: customFormatBlock?() ?? decimalPoints.format, value) + suffix.rawValue 
    }
